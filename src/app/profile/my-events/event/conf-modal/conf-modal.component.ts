...
export class ConfModalComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.conf) {
      this.preview.availableFrom = this.conf.availableFrom;
      this.preview.availableTo = this.conf.availableTo;
    } else {
  ...
  async addDocument(event) {
    ...
    this.storageService
      .uploadImage(`documents/${this.conf.id}/${selected.name}`, selected)
      .then((res) => {
        this.conf.documents.push(res);
      });

    // const x = await write_blob({
    //   directory: Directory.Documents,
    //   path: `documents/${this.conf.id}/${selected.name}`,
    //   blob: selected,
    // });
    // console.log(x);
  }
